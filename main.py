...
#    if (len(date) == 2):
#       main_bike = day_bike[(day_bike["dteday"] >= str(date[0])) & (day_bike["dteday"] <= str(date[1]))]
#    else:
#       main_bike = day_bike[
#            (day_bike["dteday"] >= str(st.session_state.date[0])) & (day_bike["dteday"] <= str(st.session_state.date[1]))]

#    workingday_bike = by_workingday(main_bike)
#    workingday(workingday_bike)

#    weathersit_bike = by_weather(main_bike)
#    weathersit(weathersit_bike)
